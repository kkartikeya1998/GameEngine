...
RenderAssetMetadata RenderAssetMetadata::fromJson(const std::string &id, const nlohmann::json &entry)
{
    ...
    meta.RenderData.sourceTileSize = entry.value("tile_size", static_cast<int>(GameConstants::TILE_SIZE));
    meta.RenderData.renderScale = entry.value("scale", RenderData::autoScale(meta.RenderData.sourceTileSize));
